...
if (isProduction && adminToken === "local-admin") {
    throw new Error("Set a strong ADMIN_TOKEN before starting the production server.");
}
...
export const config = {
    ...
    allowDevAuth,
    adminToken,
    databasePath: resolve(databasePath.startsWith("/") ? databasePath : workspaceRoot, databasePath.startsWith("/") ? "." : databasePath),
    sessionDays: 7,
