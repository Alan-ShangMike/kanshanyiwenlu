import { categories, fieldClues, gates, knowledgeCards, skills, techniques, worldHooks, elementHarbor } from "@kanshan/shared";
import { getPlayerState, ensureDiscoveryRecord, savePlayerState } from "./db.js";
...
function requireState(userId) {
    ...
    if (!player)
        throw new Error("Player state is missing.");
    return player;
}
...
function applyMutation(player, mutation) {
    ...
    if (mutation.type === "complete-category") {
        ...
        if (!category)
            throw new Error("Unknown knowledge category.");
        const available = category.prerequisiteSkillIds.every((skillId) => player.unlockedSkillIds.includes(skillId));
        if (!available)
            throw new Error("Category prerequisites are not met.");
        ...
        if (!player.completedCategoryIds.includes(category.id)) {
            ...
            addMaterial(player, category.reward.materialId, category.reward.amount);
    ...
    if (mutation.type === "open-gate") {
        ...
        if (!gate)
            throw new Error("Unknown access gate.");
        if (!gate.requiredSkillIds.every((skillId) => player.unlockedSkillIds.includes(skillId))) {
            throw new Error("Required skill is not unlocked.");
        }
        if (!gate.requiredClueIds.every((clueId) => player.inspectedClueIds.includes(clueId))) {
    ...
    if (mutation.type === "complete-daily-rift") {
        ...
        if (!player.dailyRiftCompletedIds.includes(rift.id)) {
            ...
            if (rift.reward.skillId) {
                ...
                uniquePush(player.collectibleIds, `技能残页 · ${skill?.name ?? rift.reward.skillId}`);
            }
    ...
    if (mutation.type === "discover-world-hook") {
        ...
        if (!hook)
            throw new Error("Unknown world hook.");
        if (!hook.requiredSkillIds.every((skillId) => player.unlockedSkillIds.includes(skillId))) {
            throw new Error("Required skill is not unlocked.");
        }
    ...
    if (mutation.type === "inspect-clue") {
        ...
        if (!clue)
            throw new Error("Unknown field clue.");
    ...
    if (mutation.type === "craft-technique") {
        ...
        const hasComponents = technique.components.every((component) => skills.some((skill) => skill.name === component && player.unlockedSkillIds.includes(skill.id)));
        if (!hasComponents)
            throw new Error("Technique components are not unlocked.");
...
export function applyProgressMutation(userId, mutation) {
    const player = requireState(userId);
    const wasUndiscovered = mutation.type === "discover-world-hook" &&
