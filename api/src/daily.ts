import type { DailyRift, PlayerState } from "@kanshan/shared";
import {
  categories,
  knowledgeCards,
  skills
...
const fallbackTopics: HotTopic[] = [
  {
    ...
    summary:
      "离线演示主题：观察元素、周期表与生活中的化学现象。接入知乎热榜后，这一位置会替换为当日真实热点。",
    keyPoints: [
      "先区分可以观察到的现象与尚未验证的解释。",
  ...
  },
  {
    ...
    url: "https://www.zhihu.com/hot",
    summary:
      "离线演示主题：从材料、温度、湿度与能量路径理解环境条件如何影响结果。接入知乎热榜后自动替换。",
    keyPoints: [
      "材料变化通常由多个环境变量共同决定。",
      "对照实验需要一次只改变一个主要条件。",
      "结论只能在测试条件范围内成立。"
  ...
  },
  {
    ...
    url: "https://www.zhihu.com/hot",
    summary:
      "离线演示主题：把熟悉的生活问题拆成可验证的证据。接入知乎热榜后自动替换。",
    keyPoints: [
...
function safeKeyPoints(topic: HotTopic) {
...
function buildFallbackRift(topic: HotTopic, date: string): DailyRift {
  const variants = [
    {
      puzzle: {
        kind: "logic-order" as const,
  ...
  return {
    ...
    premise:
      "周期庭院边缘出现了一段不稳定回声。它不会替玩家给出答案，只把今天的热点线索投映成一组机关。",
...
async function generateRift(topic: HotTopic, date: string) {
  const generated = await requestDeepSeekJson<DailyRift>(
    [
      "你是《看山异闻录》的关卡策划。",
  ...
  return {
    ...
    sourceTopic: {
      ...
      summary: truncate(topic.summary, 220),
      keyPoints:
        generated.sourceTopic?.keyPoints
          ?.filter((point): point is string => typeof point === "string")
          .map((point) => point.trim())
          .filter(Boolean)
          .slice(0, 4)
    ...
    reward: {
      ...
      skillId: skills.some((skill) => skill.id === generated.reward?.skillId)
        ...
        : undefined,
      skinId:
        typeof generated.reward?.skinId === "string"
...
export async function refreshDailyRift(
  force = false
): Promise<{ rift: DailyRift; source: "zhihu" | "demo"; warning: string | null }> {
  ...
  let topic: HotTopic;
  let source: "zhihu" | "demo" = "zhihu";
  let warning: string | null = null;

  try {
    const topics = await fetchZhihuHotTopics();
    if (!topics[0]) throw new Error("Hot list returned no usable topic.");
    topic = topics[0];
...
export function dailyProgress(player: PlayerState) {
