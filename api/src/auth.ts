...
import type { PersonalizationSnapshot } from "@kanshan/shared";
...
function establishSession(reply: FastifyReply, userId: string) {
...
function redirectToApp(reply: FastifyReply, query: string) {
...
export async function registerAuthRoutes(app: FastifyInstance) {
  ...
  app.get("/api/auth/zhihu/start", async (_request, reply) => {
    ...
    const browserToken =
      _request.cookies[oauthBrowserCookieName] ?? randomToken(24);
    createOAuthState(
      hashToken(state),
      hashToken(browserToken),
      Date.now() + config.oauthStateMinutes * 60 * 1000
    );
    reply.setCookie(
      oauthBrowserCookieName,
      browserToken,
      sessionCookieOptions(config.oauthStateMinutes * 60)
    );
  ...
  }>("/api/auth/zhihu/callback", async (request, reply) => {
    try {
      ...
      if (!code) return redirectToApp(reply, "auth_error=missing_code");
      ...
      try {
        personalization = await syncZhihuPersonalization(
          token.accessToken,
          profile
        );
      } catch {
      ...
      if (!user) throw new Error("Could not create user session.");
