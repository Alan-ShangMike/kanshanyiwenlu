...
function requireAdmin(
  request: Parameters<typeof isAdmin>[0],
...
app.get("/api/bootstrap", async (request, reply) => {
  ...
  if (!player) return reply.code(500).send({ error: "Player state missing." });
...
app.post<{ Body: ProgressMutation }>("/api/progress", async (request, reply) => {
  const user = requireUser(request, reply);
  if (!user) return;
  ...
  try {
    return reply.send(
      applyProgressMutation(user.id, parsed.data as ProgressMutation)
    );
  } catch (error) {
...
app.post<{ Body: { message: string } }>("/api/guide", async (request, reply) => {
  const user = requireUser(request, reply);
  if (!user) return;
...
app.post<{
  Body: { position: { x: number; y: number; z: number }; text: string };
  ...
  if (!parsed.success) return reply.code(400).send({ error: "笔记需要 4 到 180 个字。" });
...
app.post<{
  ...
  Body: { name: string };
}>("/api/discoveries/:hookId/name", async (request, reply) => {
  const user = requireUser(request, reply);
  if (!user) return;
  const parsed = z.object({ name: z.string().trim().min(2).max(24) }).safeParse(request.body);
  if (!parsed.success) return reply.code(400).send({ error: "名称需要 2 到 24 个字。" });
...
app.get("/api/admin/status", async (request, reply) => {
  ...
  return reply.send({
    ...
    runtime: {
      ...
      zhihuAccessSecretConfigured: Boolean(getZhihuAccessSecret())
    }
  });
});
...
app.put<{
  ...
  return reply.send({
    ok: true,
    integrations: integrationStatus(),
    note: "模型地址已更新。API Key 与知乎 Access Secret 只能写在服务端环境变量中。"
  });
});
