...
function eventMatches(definition: AchievementDefinition, eventKey: string) {
...
function applyReward(player: PlayerState, achievement: AchievementDefinition) {
...
export function recordProgressEvent(
  ...
  for (const achievement of achievements) {
    if (!eventMatches(achievement, eventKey)) continue;
    ...
    if (
      nextCount >= achievement.targetCount &&
...
export function describeAchievement(achievementId: string) {
