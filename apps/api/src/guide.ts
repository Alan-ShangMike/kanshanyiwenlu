...
function nextCategory(player: PlayerState) {
  const unlocked = new Set(player.unlockedSkillIds);
  return (
    categories.find(
      (category) =>
        !player.completedCategoryIds.includes(category.id) &&
        category.prerequisiteSkillIds.every((skillId) => unlocked.has(skillId))
    ) ?? null
  );
}

function localGuide(player: PlayerState, message: string): LiuGuideResponse {
  const completed = player.completedCategoryIds.length;
  const next = nextCategory(player);
  const latestSkill = skills.find((skill) =>
    player.unlockedSkillIds.includes(skill.id)
  );
  const card = next
  ...
  if (/热榜|异闻|裂隙|每日/.test(message)) {
    ...
    return {
      ...
      source: "local",
      text: rift
        ? `谢邀。今天的裂隙叫“${rift.title}”。它只借用热点的线索，不把别人的回答直接搬进游戏。你想先去看看，还是继续主线？以上。`
        : "谢邀。今天还没有生成裂隙。后台会在八点刷新，也可以手动酿造一次。别担心，我会在入口等你。以上。",
  ...
  if (!next) {
    return {
      speaker: "刘看山",
      source: "local",
      text: "谢邀。周期庭院的元素规律都醒了，但远处还有一格没有署名。今天的裂隙会先陪你，下一块地图的线索已经留在日志里。以上。",
  ...
  return {
    speaker: "刘看山",
    source: "local",
    text: `谢邀。${skillText} 下一站像是“${next.name}”：${next.description}${card ? ` 我会把《${card.title}》放进卷宗，读完后自己决定怎么破局。` : ""} 以上。`,
    suggestions: ["为什么先来这里？", "给我一个不剧透的提示", "热榜裂隙在哪里？"]
  };
}

export async function createLiuGuide(
  player: PlayerState,
  message: string
): Promise<LiuGuideResponse> {
  ...
  try {
    const generated = await requestDeepSeekText(
      [
        ...
        "回复必须使用简体中文，最多100字，可以有一点点俏皮。可偶尔用“谢邀”“以上。”，但化学引导必须清楚。",
